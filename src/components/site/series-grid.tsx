      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-obsidian">
        {model.image ? (
          <>
            ...
            />
            {/* The secondary frame, uncovered on hover rather than cross-faded
                over the first, so the two are never both half-visible. Only
                rendered when a series actually has a second photograph —
                duplicating the first would be a swap that shows nothing. */}
            {model.gallery?.[0] && (
              <Image
                src={model.gallery[0]}
                alt=""
                ...
                quality={90}
                sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 460px"
                className="reveal-swap object-cover"
              />
            )}
            <div
              aria-hidden
